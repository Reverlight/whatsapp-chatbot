...
def send_main_menu(phone: str) -> None:
    _post({
        ...
        "interactive": {
            "type": "list",
            "body": {"text": f"👋 Welcome to {settings.RESTAURANT_NAME}!\n\nHow can we help you today?"},
            "action": {
                "button": "Open Menu",
                "sections": [{
                    "title": "Options",
                    "rows": [
                        ...
                        {"id": "suggestions", "title": "🤖 AI Suggestions",   "description": "Get personalised recommendations"},
                        {"id": "info",        "title": "ℹ️ Restaurant Info",  "description": "Address, hours & contacts"},
                    ],
                }],
            },
        },
...
def send_contact_menu(phone: str) -> None:
    _post({
        ...
        "interactive": {
            ...
            "action": {
                "buttons": [
                    {"type": "reply", "reply": {"id": "chat_admin", "title": "💬 Chat with Admin"}},
                    {"type": "reply", "reply": {"id": "get_info",   "title": "📋 Get Contact Info"}},
...
def send_cancel_reservation_menu(phone: str, date_str: str, guests: int) -> None:
    _post({
        ...
        "interactive": {
            ...
            "action": {
                "buttons": [
                    {"type": "reply", "reply": {"id": "cancel_reservation", "title": "❌ Cancel it"}},
                    {"type": "reply", "reply": {"id": "back",               "title": "⬅️ Keep it"}},
                ],
            },
        },
    })
...
def forward_to_admins(customer_phone: str, text: str) -> None:
    ...
    for admin in settings.ADMIN_PHONES:
        _post({
            ...
            "to": admin,
            "type": "text",
            "text": {"body": f"📩 *Support message from +{customer_phone}:*\n\n{text}"},
        })


def send_admin_reply_to_client(customer_phone: str, admin_phone: str, reply_text: str) -> None:
